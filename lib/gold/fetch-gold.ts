...
import { computeGoldPnlSnapshot } from "@/lib/gold/pnl";
...
export async function fetchParentGoldSavingsData(
  ...
  const [holdingsResult, pendingResult, transactionsResult] = await Promise.all([
    ...
    supabase.from("gold_holdings").select("profile_id, quantity_milli").in("profile_id", profileIds),
  ...
  for (const row of (holdingsResult.data ?? []) as { profile_id: string; quantity_milli: number }[]) {
...
export async function fetchChildGoldSavingsData(
  ...
  const pnlHistory = mapGoldTransactions((pnlHistoryResult.data ?? []) as GoldTransactionDbRow[]);

  const pnl = computeGoldPnlSnapshot(

    pnlHistory,

    quantityMilli,

    settings.buyPriceEnergy,

    settings.sellPriceEnergy,
