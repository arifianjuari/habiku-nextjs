-- Tabungan dari dompet: hanya energi di target aktif (sum goals.current_hp status active),
-- bukan seluruh saldo point_ledger.

create or replace function public.compute_savable_goal_energy (p_profile_id uuid)
returns int
language sql
stable
security invoker
...
grant execute on function public.compute_savable_goal_energy (uuid) to authenticated;
