-- Ortu boleh koreksi durasi kunci deposito saat kantong sudah berisi saldo.
-- locked_until dihitung ulang dari tanggal setoran awal + bulan kunci baru.
...
begin
  ...
  if v_balance = 0 then
    ...
    v_new_lock_months := case when v_new_type = 'term' then least(36, p_lock_months) else null end;
    v_new_lock_coef := greatest(0.1, least(5.0, coalesce(p_lock_bonus_coefficient, 1.0)));
  else
    v_new_type := v_pocket.pocket_type;
    v_new_bps := greatest(0, coalesce(p_monthly_interest_bps, v_pocket.monthly_interest_bps));
    if v_new_bps > 2000 then
      raise exception 'interest_rate_too_high' using errcode = 'P0001';
    end if;
    if v_pocket.pocket_type = 'term' then
      v_new_lock_months := least(36, greatest(1, coalesce(p_lock_months, v_pocket.lock_months)));
      v_new_lock_coef := greatest(0.1, least(5.0, coalesce(p_lock_bonus_coefficient, v_pocket.lock_bonus_coefficient)));
    else
  ...
  if v_pocket.pocket_type = 'term' and v_balance > 0 and v_new_lock_months is not null then
    update public.savings_transactions t
    set locked_until = t.created_at + (v_new_lock_months || ' months')::interval
