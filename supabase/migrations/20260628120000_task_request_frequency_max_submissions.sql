-- Frekuensi & batas pengerjaan yang diajukan anak pada task_requests.

alter table public.task_requests
  add column if not exists requested_frequency_type public.frequency_type not null default 'daily';

alter table public.task_requests
  add column if not exists requested_max_submissions_per_period int not null default 1;

alter table public.task_requests
  drop constraint if exists task_requests_max_submissions_positive;

alter table public.task_requests
  add constraint task_requests_max_submissions_positive
  check (requested_max_submissions_per_period >= 1);

comment on column public.task_requests.requested_frequency_type is
  'Frekuensi misi yang diajukan anak (daily/weekly); ortu dapat menyesuaikan saat menyetujui.';

comment on column public.task_requests.requested_max_submissions_per_period is
  'Batas pengerjaan per periode yang diajukan anak; ortu dapat menyesuaikan saat menyetujui.';

-- Perluas RPC pengajuan misi dari mode anak.
drop function if exists public.submit_task_request (uuid, text, text, int);
...
revoke all on function public.submit_task_request (
  uuid,
  text,
  text,
  int,
...
grant execute on function public.submit_task_request (
  uuid,
  text,
  text,
  int,
...
comment on function public.submit_task_request (
  ...
  'Ajukan ide misi dari mode anak; termasuk energi, frekuensi, dan batas pengerjaan per periode.';
