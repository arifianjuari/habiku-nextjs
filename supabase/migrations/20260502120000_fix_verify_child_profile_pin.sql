-- Perbaikan: pgcrypto hidup di schema `extensions` — kualifikasi `extensions.crypt` agar
--           tetap benar saat `SECURITY DEFINER` (search_path). Tanpa RAISE: supaya salah PIN
--           = return false, bukan error gRPC.
...
comment on function public.verify_child_profile_pin (uuid, text) is
  'Ortu (sesi): cek PIN profil anak (bcrypt via extensions.crypt).';
