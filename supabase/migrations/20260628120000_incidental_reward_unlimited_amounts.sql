-- Reward insidental: hapus batas atas 50 pada HP ke target dan energi bebas.

alter table public.incidental_rewards
  drop constraint if exists incidental_rewards_hp_to_target_check;

alter table public.incidental_rewards
  drop constraint if exists incidental_rewards_energy_only_check;

alter table public.incidental_rewards
  add constraint incidental_rewards_hp_to_target_check check (hp_to_target >= 0);

alter table public.incidental_rewards
  add constraint incidental_rewards_energy_only_check check (energy_only >= 0);
